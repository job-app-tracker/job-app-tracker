<template>
  <v-card min-width="400">
    <v-layout row wrap pa-4>
      ...
      <v-flex xs12 class="pl-4 pr-4">
        <v-text-field v-model="new_job_company" outline label="Company Name">
        </v-text-field>
      </v-flex>
      <v-flex xs12 class="pl-4 pr-4">
        <v-text-field v-model="new_job_title" outline label="Job Title">
        </v-text-field>
      </v-flex>
      <v-flex xs12 class="pl-4 pr-4 text-xs-center">
        <v-btn round outline color="red" v-on:click="addNewJob()"
          >Add Job</v-btn
        >
      </v-flex>
    </v-layout>
